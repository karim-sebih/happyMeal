window.openModal = function (recipe) {
    console.log("Données de la recette :", recipe); // Vérifier si la recette est bien passée
    const modal = document.getElementById("recipeModal");
    
    if (!modal) {
        console.error("❌ Le modal ne peut pas être ouvert car il n'existe pas.");
        return;
    }

    document.getElementById("modal-title").textContent = recipe.nom || "Recette inconnue";

    const modalImage = document.getElementById("modal-image");
    modalImage.src = recipe.image?.src || recipe.image || "default.jpg";
    modalImage.alt = recipe.image?.alt || "Image de la recette";

    document.getElementById("modal-time").textContent = recipe.temps_preparation || "Non précisé";
