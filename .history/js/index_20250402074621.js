...
//Barre de Recherchedocument.addEventListener("DOMContentLoaded", async function () {
    ...
    function showSuggestions(query) {
        ...
        if (query.length < 2) return; // Commencer la recherche après 2 caractères

        const results = recettes.filter(recipe =>
            recipe.nom.toLowerCase().includes(query.toLowerCase()) ||
        ...
        results.forEach(recipe => {
            ...
            li.addEventListener("click", () => {
                searchBar.value = recipe.nom;
                suggestionsList.innerHTML = "";
                openModal(recipe); // Ouvrir la pop-up avec les détails de la recette
            });
    ...
    // Cacher la liste lorsque l'on clique en dehors
