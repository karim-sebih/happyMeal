...
const allRecipes = Object.values(categories).flat(); // Fusionne toutes les recettes en un seul tableau
const totalPages = Math.ceil(allRecipes.length / recipesPerPage);


function displayPaginatedRecipes(page) {
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        const response = await fetch("../json/data.json");
        const data = await response.json();
        allRecipes = data.recettes; // Récupère toutes les recettes sans distinction de catégorie
        ...
        document.getElementById("prev-btn").addEventListener("click", function () {
            if (currentPage > 1) {
                currentPage--;
                displayPaginatedRecipes(currentPage);
            }
        });
...
document.addEventListener("DOMContentLoaded", async function () {
    const container = document.querySelector(".recettes-container");
    console.log("Vérification container :", container);

    if (!container) {
        console.error("Erreur : Le conteneur .recettes-container n'existe pas !");
    ...
    try {
        ...
        function displayPaginatedRecipes(page) {
            container.innerHTML = ""; // Vérifie que container n'est pas null
            const start = (page - 1) * recipesPerPage;
            const end = start + recipesPerPage;
            const paginatedRecipes = allRecipes.slice(start, end);
