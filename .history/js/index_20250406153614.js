...
document.addEventListener("DOMContentLoaded", function () {
    ...
    favoriteBtn.addEventListener("click", function () {
        ...
        // Trouver la recette complète dans les données chargées
        const recipe = recipes.find(r => r.nom === mealName); // Assurez-vous que "recipes" est accessible
        const index = favorites.findIndex(fav => fav.nom === mealName);

        if (index === -1) {
            // Ajouter la recette complète si elle n'est pas déjà en favori
            favorites.push(recipe);
        } else {
            // Supprimer la recette si elle est déjà en favori
