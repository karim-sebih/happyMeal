...
function openModal(recipe) {
    ...
    recipe.ingredients.forEach(ing => {
        ...
        let ingredientName = ingredientText.replace(/^\d+\s*[a-zA-Z]*\s*-\s*/, "").trim();
...
// Fonction pour ajouter un ingrédient à la liste de courses (sans quantité)
...
function displayShoppingList() {
    ...
    shoppingList.forEach(item => {
        const li = document.createElement("li");
        li.textContent = item; // Affiche uniquement le nom de l'ingrédient

        // Bouton pour supprimer l'ingrédient
        const removeBtn = document.createElement("button");
        removeBtn.textContent = "❌";
        removeBtn.classList.add("remove-item");
        removeBtn.dataset.ingredient = item;

        removeBtn.addEventListener("click", function () {
            removeIngredientFromShoppingList(item);
        });
...
// Ajout d'un event listener sur les ingrédients affichés dans la pop-up
document.addEventListener("click", function (event) {
    if (event.target.closest("#modal-ingredients li")) {
        let ingredientText = event.target.textContent;

        // Si l'ingrédient contient une quantité (ex: "50g Parmesan"), on prend juste le nom
        let ingredientName = ingredientText.replace(/^\d+\s*[a-zA-Z]*\s*-\s*/, "").trim();
