...
function openModal(recipe) {
    ...
    recipe.ingredients.forEach(ing => {
        ...
        // Vérifie si l'ingrédient est sous forme d'objet ou non
        let ingredientText = (typeof ing === "object") ? `${ing.quantite} - ${ing.nom}` : ing;
    
        li.innerHTML = `
            ${ingredientText} 
            <button class="add-to-list" data-ingredient="${ingredientText}">➕</button>
        `;
    
        ingredientsList.appendChild(li);
    });
    
    // Ajoute des event listeners aux boutons "Ajouter"
...
// Fonction pour afficher les ingrédients avec bouton "Ajouter"
function generateIngredientList(recipeName) {
    const recipe = fetchedRecipes.find(r => r.nom === recipeName);
    const ingredientsList = document.getElementById('modal-ingredients');
    ingredientsList.innerHTML = ''; // Vide la liste actuelle

    recipe.ingredients.forEach(ingredient => {
        const li = document.createElement('li');
        li.innerHTML = `
            ${ingredient.quantite} ${ingredient.nom} 
            <button class="add-to-list" data-ingredient="${ingredient.quantite} ${ingredient.nom}">➕</button>
        `;
        ingredientsList.appendChild(li);
    });

    // Ajoute un event listener aux boutons d'ajout
    document.querySelectorAll('.add-to-list').forEach(button => {
        button.addEventListener('click', function () {
            addIngredientToShoppingList(this.dataset.ingredient);
        });
...
function addIngredientToShoppingList(ingredient) {
    ...
    displayShoppingList(); // Met à jour l'affichage
}
...
function displayShoppingList() {
    ...
    shoppingListContainer.innerHTML = shoppingList.map(item => `
        <li>${item} <button class="remove-item" data-ingredient="${item}">❌</button></li>
    `).join('');

    // Ajoute un event listener aux boutons de suppression
