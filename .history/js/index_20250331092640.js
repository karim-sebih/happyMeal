...
function openModal(recipe) {
    ...
    recipe.ingredients.forEach(ing => {
        ...
        // Extraction du nom de l’ingrédient sans la quantité
        let ingredientName;
        if (typeof ing === "object") {
            ingredientName = ing.nom; // Si c’est un objet avec "nom"
        } else {
            // Si c’est une chaîne (ex. "2 cl - eau"), on supprime la quantité
            ingredientName = ing.replace(/^\d+\s*[a-zA-Z]*\s*-\s*/, "").trim();
        }
        ...
        // Ajout des éléments à la liste (affichage avec quantité pour lisibilité)
        li.appendChild(checkbox);
        li.appendChild(document.createTextNode(" " + (typeof ing === "object" ? `${ing.quantite} - ${ing.nom}` : ing)));
...
function addIngredientToShoppingList(ingredient) {
    console.log("Ajout de l'ingrédient :", ingredient);
}

// Ensuite, l'utiliser dans un événement
document.addEventListener("DOMContentLoaded", function () {
    const checkbox = document.querySelector("input[type='checkbox']");
    checkbox.addEventListener("change", function () {
        if (this.checked) {
            addIngredientToShoppingList(this.dataset.ingredient); // Appel correct
        }
    });
});