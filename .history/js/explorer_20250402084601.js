document.addEventListener("DOMContentLoaded", async function () {
    ...
    const pagination = { "Plat principal": 0, "Entrée": 0, "Dessert": 0 };
    
    function createPaginationButtons(container, category, recipes) {
        const paginationContainer = document.createElement("div");
        paginationContainer.className = "pagination";
        
        const prevButton = document.createElement("button");
        prevButton.textContent = "Précédent";
        prevButton.disabled = pagination[category] === 0;
        prevButton.addEventListener("click", function () {
            if (pagination[category] > 0) {
                pagination[category]--;
                displayRecipes(container, category, recipes);
            }
        });
        
        const nextButton = document.createElement("button");
        nextButton.textContent = "Suivant";
        nextButton.disabled = (pagination[category] + 1) * paginationLimit >= recipes.length;
        nextButton.addEventListener("click", function () {
            if ((pagination[category] + 1) * paginationLimit < recipes.length) {
                pagination[category]++;
                displayRecipes(container, category, recipes);
            }
        });
    ...
    try {
        ...
        function displayRecipes(container, category, recipes) {
            container.innerHTML = "";
            
            const start = pagination[category] * paginationLimit;
            ...
            createPaginationButtons(container, category, recipes);
        }

        displayRecipes(principalContainer, "Plat principal", categories["Plat principal"]);
        displayRecipes(entreeContainer, "Entrée", categories["Entrée"]);
        displayRecipes(dessertContainer, "Dessert", categories["Dessert"]);
